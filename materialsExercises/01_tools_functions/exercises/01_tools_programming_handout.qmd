...
```
C:/Users/anna/Documents/r_course/data/swiss.csv     (Windows)
/Users/anna/Documents/r_course/data/swiss.csv       (Mac)
```

A **relative path** starts from wherever you currently are:

```
data/swiss.csv     the file swiss.csv, in the folder data, inside the current folder
...
setwd("C:/Users/anna/Documents/r_course")   # move somewhere else
```
...
1. In your file browser, create a folder called `r_course`, for example in Documents.
2. Inside it, create two subfolders, `data` and `code`.
3. In RStudio, click the **Project** button in the top-right corner, choose
   **New Project... > Existing Directory**, browse to `r_course`, and click
   **Create Project**.
4. Check with `getwd()`. The path should end in `/r_course`.
:::
...
- `source("code/my_script.R")` runs a whole script from the Console
...
4. Put the cursor on that line and press `Ctrl+Enter` (`Cmd+Enter`). Watch it appear in
   the Console and run.
5. Save the file into the `code` subfolder as `hello_world.R`.
...
3. Create a vector `grades` holding 4.5, 5, 5.5, 4 and 6. How many values does it have?
   What is the average?
4. Type `Age`, with a capital A. What happens, and why?
:::
